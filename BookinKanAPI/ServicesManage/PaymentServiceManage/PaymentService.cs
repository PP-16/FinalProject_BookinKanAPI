...
namespace BookinKanAPI.ServicesManage.PaymentServiceManage
{
    public class PaymentService : IPaymentService
    {
        ...
        public async Task<object> CreateOrUpdatePayment(PaymentDTO payment)
        {
            ...
            var orderPastPayment = await _dataContext.OrdersPastDues.FirstOrDefaultAsync(p => p.OrdersPastDueId == payment.OrdersPastDueId);
            var order = await _dataContext.OrderRents.Include(i => i.OrderRentItems).ThenInclude(d=>d.Driver).FirstOrDefaultAsync(p => p.OrderRentId == payment.OrderRentId);
            ...
            if (payment.CategoryPayment == 2)
            {
                ...
                var options = new PaymentIntentCreateOptions
                {
                    Amount = booking is not null ? booking.TotalPrice * 100
                : order is not null ? order.GetTotalPrice() * 100
                : orderPastPayment.TotalPricePastDue * 100 ,  // Assuming the amount is in cents
                    Currency = "THB",
                    PaymentMethodTypes = new List<string> { "card" }
                };
                ...
                var newPaymentBooking = new PaymentBooking
                {
                    ...
                    OrdersPastDueId = payment.OrdersPastDueId,
        ...
        public async Task<object> UpdatePaymentStripe(int Id, int PaymentId)
        {
            ...
            var orderPastPayment = await _dataContext.OrdersPastDues.FirstOrDefaultAsync(p => p.OrdersPastDueId == Id);

            var order = await _dataContext.OrderRents.Include(i => i.OrderRentItems).FirstOrDefaultAsync(p => p.OrderRentId == Id);

            var options = new PaymentIntentUpdateOptions
            {
                Amount = bookingPayment is not null ? bookingPayment.Booking.TotalPrice * 100
                : orderPayment is not null ? order.GetTotalPrice() * 100
                : orderPastPayment is not null ? orderPastPayment.TotalPricePastDue * 100 : null // Assuming the amount is in cents
